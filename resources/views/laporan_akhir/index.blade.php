                    ...
                    <div class="card-header border-0 pt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" id="myInput" class="form-control form-control-solid w-250px ps-15" placeholder="Search Laporan Akhir" name="search" />
                            </div>
                        </div>
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('laporan-akhir.create') }}" class="btn btn-primary">Tambah Laporan Akhir</a>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($laporan->dokumen_laporan_akhir)
                                                <a href="{{ asset('storage/' . $laporan->dokumen_laporan_akhir) }}" target="_blank">Download</a>
                                            @else
                                                Tidak ada dokumen
                                            @endif
                                        </td>
                                        <td class="text-end">
                                            <a href="{{ route('laporan-akhir.edit', $laporan->id) }}" class="btn btn-light btn-active-light-primary btn-sm">Edit</a>
                                            <form action="{{ route('laporan-akhir.destroy', $laporan->id) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
